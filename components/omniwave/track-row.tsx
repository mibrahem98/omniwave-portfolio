...
export function TrackRow({ track, active, onPress, onFavorite, onQueue, trailing }: TrackRowProps) {
  ...
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.row, compact && styles.rowCompact, { flexDirection: isRTL ? "row-reverse" : "row" }, active && { backgroundColor: theme.colors.glassOverlay, paddingHorizontal: 8, marginHorizontal: -8, borderColor: `${track.accent}4D`, borderWidth: 1 }, pressed && (reduceMotion ? styles.pressedReduced : styles.pressed)]}>
...
const styles = StyleSheet.create({
  ...
  pressed: { opacity: 0.76, transform: [{ scale: 0.988 }] }, pressedReduced: { opacity: 0.76 },
