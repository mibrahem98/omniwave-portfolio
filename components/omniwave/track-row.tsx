...
export function TrackRow({ track, active, onPress, onFavorite, onQueue, trailing }: TrackRowProps) {
  const { theme, isRTL, t, interfaceDensity, textScaleMultiplier, fontWeightValue } = useThemeContext();
  ...
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.row, compact && styles.rowCompact, { flexDirection: isRTL ? "row-reverse" : "row" }, active && { backgroundColor: `${theme.colors.primary}15`, paddingHorizontal: 8, marginHorizontal: -8 }, pressed && styles.pressed]}>
      ...
      <View style={styles.copy}>
        <Text numberOfLines={1} style={[styles.title, { color: active ? theme.colors.primary : theme.colors.text, textAlign, fontSize: scaled(15), lineHeight: scaled(21), fontWeight: fontWeightValue }]}>{track.title}</Text>
        <Text numberOfLines={1} style={[styles.subtitle, { color: theme.colors.muted, textAlign, fontSize: scaled(12), lineHeight: scaled(18), fontWeight: fontWeightValue }]}>{track.artist} <Text style={{ color: theme.colors.border }}>·</Text> {track.album}</Text>
        {track.classificationStatus === "pending" ? <Text numberOfLines={1} style={[styles.classification, { color: theme.colors.primary, textAlign, fontSize: scaled(10), lineHeight: scaled(14), fontWeight: fontWeightValue }]}>{t("classificationPending")}</Text> : track.classificationStatus === "suggested" ? <Text numberOfLines={1} style={[styles.classification, { color: theme.colors.secondary, textAlign, fontSize: scaled(10), lineHeight: scaled(14), fontWeight: fontWeightValue }]}>{t("reviewRequired")}</Text> : track.classification ? <Text numberOfLines={1} style={[styles.classification, { color: theme.colors.secondary, textAlign, fontSize: scaled(10), lineHeight: scaled(14), fontWeight: fontWeightValue }]}>{track.classification.genre} <Text style={{ color: theme.colors.border }}>·</Text> {track.classification.mood}</Text> : track.classificationStatus === "failed" ? <Text numberOfLines={1} style={[styles.classification, { color: theme.colors.accent, textAlign, fontSize: scaled(10), lineHeight: scaled(14), fontWeight: fontWeightValue }]}>{t("classificationFailed")}</Text> : null}
      </View>
      {trailing ?? (
        <View style={[styles.trailing, { flexDirection: isRTL ? "row-reverse" : "row" }]}>
          <Text style={[styles.duration, { color: theme.colors.muted, fontSize: scaled(11), lineHeight: scaled(16), fontWeight: fontWeightValue }]}>{track.durationSeconds ? formatTime(track.durationSeconds) : t("yourFiles")}</Text>
