...
export function MiniPlayer() {
  ...
  return <Reveal style={styles.reveal}><GestureDetector gesture={swipeGesture}><View collapsable={false}><Pressable accessibilityRole="button" accessibilityLabel={`${t("nowPlaying")}: ${currentTrack.title}`} accessibilityHint={`${t("previous")} / ${t("next")}`} onPress={() => router.push("/(tabs)/now-playing" as never)} style={({ pressed }) => [styles.shell, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border, flexDirection: isRTL ? "row-reverse" : "row" }, pressed && styles.pressed]}>
    <View style={[styles.progressRail, { backgroundColor: theme.colors.surfaceMuted }]}><View style={[styles.progressValue, { width: `${progress}%`, backgroundColor: theme.colors.primary }]} /></View>
    ...
    <Pressable accessibilityRole="button" accessibilityLabel={t("previous")} onPress={(event) => { event.stopPropagation(); previousTrack(); }} style={({ pressed }) => [styles.skipButton, { backgroundColor: theme.colors.surfaceMuted }, pressed && styles.pressed]}><MaterialIcons name="skip-previous" size={19} color={theme.colors.text} /></Pressable>
    ...
    <Pressable accessibilityRole="button" accessibilityLabel={t("next")} onPress={(event) => { event.stopPropagation(); nextTrack(); }} style={({ pressed }) => [styles.skipButton, { backgroundColor: theme.colors.surfaceMuted }, pressed && styles.pressed]}><MaterialIcons name="skip-next" size={19} color={theme.colors.text} /></Pressable>
  </Pressable></View></GestureDetector></Reveal>;
}

const styles = StyleSheet.create({
  reveal: { marginHorizontal: 14, marginBottom: 9 }, shell: { alignItems: "center", gap: 8, padding: 10, borderRadius: 23, borderWidth: 1, overflow: "hidden", shadowColor: "#000", shadowOpacity: 0.28, shadowRadius: 20, shadowOffset: { width: 0, height: 6 }, elevation: 7 },
