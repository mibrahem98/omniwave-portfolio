...
import { APP_THEMES, type AppThemeId, type HighContrastAccent, type InterfaceDensity, type TextScale, useThemeContext } from "@/lib/theme-provider";
        ...
        ListHeaderComponent={
          <View>
            ...
            <AccessibilityFeedback pulseKey={accessibilityPulse}><View style={[styles.pickerCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
              ...
              <View accessibilityRole="radiogroup" style={[styles.choiceRow, { flexDirection: direction }]}>{TEXT_SCALES.map((scale) => { const selected = textScale === scale; return <Pressable key={scale} accessibilityRole="radio" accessibilityLabel={textScaleLabels[scale]} accessibilityState={{ selected }} onFocus={pulseAccessibility} onPress={() => { setTextScale(scale); pulseAccessibility(); }} style={({ pressed }) => [styles.choice, { borderColor: selected ? theme.colors.accent : theme.colors.border, backgroundColor: selected ? `${theme.colors.accent}16` : theme.colors.surfaceMuted }, pressed && styles.pressed]}><Text style={[styles.choiceText, { color: selected ? theme.colors.accent : theme.colors.muted }]}>{textScaleLabels[scale]}</Text></Pressable>; })}</View>
