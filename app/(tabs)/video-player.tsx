...
function CaptionChoice({ selected, label, color, onPress }: { selected: boolean; label: string; color?: string; onPress: () => void }) {
  ...
  return <Pressable accessibilityRole="radio" accessibilityLabel={label} accessibilityState={{ selected }} onPress={onPress} style={({ pressed }) => [styles.captionChoice, { borderColor: selected ? theme.colors.primary : theme.colors.border, backgroundColor: selected ? `${theme.colors.primary}16` : theme.colors.surfaceMuted }, pressed && styles.pressed]}>{color ? <View style={[styles.colorDot, { backgroundColor: color }]} /> : null}{selected ? <MaterialIcons name="check" size={15} color={theme.colors.primary} /> : null}<Text style={[styles.choiceText, { color: selected ? theme.colors.primary : theme.colors.muted }]}>{label}</Text></Pressable>;
}
...
export default function VideoPlayerScreen() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (typeof initial === "number") void Brightness.setBrightnessAsync(initial).catch(() => undefined);
    };
  }, []);
  useEffect(() => {
    if (Platform.OS === "web") return;
  ...
  const restorePortrait = () => {
    if (Platform.OS === "web") return;
    void ScreenOrientation.lockAsync(ScreenOrientation.OrientationLock.PORTRAIT_UP).then(() => setLandscapeLocked(false)).catch(() => undefined);
  };
  const toggleLandscape = () => {
    ...
    void ScreenOrientation.lockAsync(ScreenOrientation.OrientationLock.LANDSCAPE).then(() => { setLandscapeLocked(true); haptic.medium(); }).catch(() => undefined);
  };
  ...
  return <ScreenContainer edges={wideLayout ? ["top", "bottom", "left", "right"] : ["top", "left", "right"]} className={wideLayout ? "" : "px-5"}><View style={[styles.screen, wideLayout && styles.wideScreen]}>
    {!wideLayout ? <View style={[styles.header, { flexDirection: direction }]}>
      <Pressable accessibilityRole="button" accessibilityLabel={t("close")} onPress={() => router.back()} style={[styles.headerButton, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}><MaterialIcons name={isRTL ? "arrow-forward" : "arrow-back"} size={21} color={theme.colors.text} /></Pressable>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel={t("fullscreen")} onPress={enterFullscreen} style={[styles.headerButton, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}><MaterialIcons name="fullscreen" size={21} color={theme.colors.text} /></Pressable>
    </View> : null}
    <View onLayout={(event) => { setFrameWidth(event.nativeEvent.layout.width); setFrameHeight(event.nativeEvent.layout.height); }} style={[styles.videoFrame, compact && styles.videoFrameCompact, wideLayout && styles.wideVideoFrame, { backgroundColor: "#000000", borderColor: theme.colors.border }]}>
      <GestureDetector gesture={mediaGestures}><View collapsable={false} style={styles.videoGestureSurface}><VideoView ref={videoViewRef} player={player} style={styles.video} nativeControls={false} contentFit={wideLayout ? "cover" : "contain"} allowsFullscreen fullscreenOptions={{ enable: true }} allowsPictureInPicture={Platform.OS !== "web"} surfaceType="textureView" onFullscreenEnter={() => { if (Platform.OS !== "web") { void ScreenOrientation.lockAsync(ScreenOrientation.OrientationLock.LANDSCAPE).catch(() => undefined); } setLandscapeLocked(true); }} onFullscreenExit={restorePortrait} /></View></GestureDetector>
